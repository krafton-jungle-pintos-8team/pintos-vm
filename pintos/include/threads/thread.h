...
struct thread {
	...
	bool is_waited;						/* wait가 호출된 적이 있는가? */
	// struct lock *process_wait_lock;		/* wait에서 사용할 lock */
	struct semaphore *fork_sema;		/* 자식 프로세스의 종료를 기다린다. */
	struct semaphore *wait_sema;		/* 자식이 종료될 때까지 wait에서 대기 */
	struct semaphore *exit_sema;

	struct intr_frame *parent_if;

	struct file *fdt[FILE_LIMIT];			/* 이중 포인터로 배열의 next_fd로 인덱스를 사용 */
	int fd;
    
    struct list child_list;             /* 자식 프로세스의 리스트 */
    struct list_elem child_elem;        /* 자식 프로세스가 부모 프로세스에 포함되기 위한 구조체? */
