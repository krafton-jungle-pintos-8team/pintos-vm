...
static void populate_pools(struct area *base_mem, struct area *ext_mem) {
    ...
    for (i = 0; i < mb_info->mmap_len / sizeof(struct e820_entry); i++) {
        ...
        if (entry->type == ACPI_RECLAIMABLE || entry->type == USABLE) {
            ...
            start = (uint64_t)pg_round_up(start >= usable_bound ? start : usable_bound);
        split:
...
uint64_t palloc_init(void) {
    ...
    printf("\tbase_mem: 0x%llx ~ 0x%llx (Usable: %'llu kB)\n", base_mem.start, base_mem.end,
           base_mem.size / 1024);
    printf("\text_mem: 0x%llx ~ 0x%llx (Usable: %'llu kB)\n", ext_mem.start, ext_mem.end,
           ext_mem.size / 1024);
    populate_pools(&base_mem, &ext_mem);
    return ext_mem.end;
}

/* PAGE_CNT개의 연속된 빈 페이지들을 확보하고 반환한다.
   PAL_USER가 설정되어 있으면 사용자 풀에서 페이지를 가져오고, 그렇지 않으면 커널 풀에서 가져온다.
   FLAGS에 PAL_ZERO가 설정되어 있으면 페이지들은 0으로 채워진다.
   사용 가능한 페이지가 부족하면 null 포인터를 반환하며, 
   FLAGS에 PAL_ASSERT가 설정되어 있는 경우에는 커널이 패닉에 빠진다.
*/
...
/* 하나의 빈 페이지를 확보하여 해당 페이지의 커널 가상 주소를 반환한다.
PAL_USER가 설정되어 있으면 사용자 풀에서 페이지를 가져오고, 그렇지 않으면 커널 풀에서 가져온다.
FLAGS에 PAL_ZERO가 설정되어 있으면 해당 페이지는 0으로 채워진다.
사용 가능한 페이지가 없으면 null 포인터를 반환하며, 
FLAGS에 PAL_ASSERT가 설정되어 있는 경우에는 커널이 패닉에 빠진다.
*/
