...
/* NEW를 해시 테이블 H에 삽입하고,
동등한(equal) 요소가 테이블에 이미 존재하지 않으면
null 포인터를 반환합니다. (즉, 삽입이 성공한 경우)
만약 동등한 요소가 이미 테이블에 존재한다면
NEW는 삽입하지 않고, 그 기존 요소를 반환합니다. */
...
hash_replace(), hash_delete() 같은 함수로 수정하면, 
모든 반복자(iterator)가 무효화됩니다.*/
struct hash_elem *hash_next(struct hash_iterator *i) {
    ...
    while (i->elem == list_elem_to_hash_elem(list_end(i->bucket))) {
        ...
        */
        ...
        // 현재 bucket리스트의 다음 bucket의 처음 elem의 주소
...
static void rehash(struct hash *h) {
    ...
    if (new_bucket_cnt < 4)
        new_bucket_cnt = 4;
    while (!is_power_of_2(new_bucket_cnt)) new_bucket_cnt = turn_off_least_1bit(new_bucket_cnt);
